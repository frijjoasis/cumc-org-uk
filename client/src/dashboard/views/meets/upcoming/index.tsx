      ...
      <div className="grid gap-6">
        {sortedMeets.length > 0 ? (
          sortedMeets.map(meet => {
            const signupCount = meet.signupCount || 0;
            const isFull =
              meet.maxSignups !== null && signupCount >= meet.maxSignups;
                ...
                <div className="flex flex-col md:flex-row">
                  ...
                  <CardHeader className="flex-1 p-8">
                    <div className="flex justify-between items-start mb-4">
                      ...
                      {meet.disabled || isFull ? (
                        <Badge className="bg-zinc-200 text-zinc-500 uppercase text-[9px] font-black">
                          {isFull ? 'Sold Out / Full' : 'Closed'}
                        </Badge>
                      ) : (
                        <Badge className="bg-emerald-500 text-white uppercase text-[9px] font-black animate-pulse">
                          Open (
                          {meet.maxSignups
                            ? `${meet.maxSignups - signupCount} left`
                            : 'Open'}
                          )
                        </Badge>
                      )}
