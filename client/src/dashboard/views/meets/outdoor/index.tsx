...
const OutdoorMeets = () => {
  return (
    <div className="max-w-7xl mx-auto space-y-12 pb-12">
      ...
      {/* Narrative Section */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-12 px-4 lg:px-0 items-center">
        <div className="lg:col-span-2 space-y-6">
          ...
          <p className="text-zinc-600 leading-relaxed text-lg whitespace-pre-line font-medium">
            {outdoorMeets}
          </p>
          <div className="flex flex-wrap gap-4 pt-4">
            <Button asChild size="lg" className="bg-zinc-900 font-black uppercase italic tracking-widest gap-2">
        ...
        <div className="space-y-4">
          {[
            { icon: Mountain, title: "Classic Trad", desc: "Peak District, Wales & Lakes" },
            { icon: Compass, title: "Sport Trips", desc: "Portland, Cheddar & Beyond" },
            { icon: Compass, title: "Bouldering", desc: "Fontainebleau & Local Crags" }
          ].map((feature, i) => (
            <Card key={i} className="border-none bg-zinc-50 shadow-none">
              <CardContent className="p-4 flex items-center gap-4">
                <div className="p-3 bg-white rounded-xl shadow-sm">
                  <feature.icon className="h-5 w-5 text-primary" />
                </div>
                <div>
                  <h4 className="font-black uppercase italic text-sm tracking-tight">{feature.title}</h4>
      ...
      <section className="grid grid-cols-1 md:grid-cols-2 gap-8 px-4 lg:px-0">
        <div className="group relative overflow-hidden rounded-2xl aspect-[4/3] shadow-lg">
          ...
          <div className="absolute bottom-4 left-4 right-4 bg-white/90 backdrop-blur-md p-3 rounded-xl transform translate-y-20 group-hover:translate-y-0 transition-transform duration-300">
            <p className="text-center font-bold text-zinc-900 uppercase text-xs">Pushing limits on the bolt</p>
          </div>
        </div>
        ...
        <div className="group relative overflow-hidden rounded-2xl aspect-[4/3] shadow-lg">
          ...
          <div className="absolute bottom-4 left-4 right-4 bg-white/90 backdrop-blur-md p-3 rounded-xl transform translate-y-20 group-hover:translate-y-0 transition-transform duration-300">
            <p className="text-center font-bold text-zinc-900 uppercase text-xs">Pebble wrestling at its finest</p>
          </div>
        </div>
      </section>
    </div>
