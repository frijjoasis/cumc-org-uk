...
import { Container, Row, Col, Card, Table, Button, Modal, Form, Alert, Nav, Badge } from 'react-bootstrap';
...
class CommitteeManager extends Component {
    ...
    handleShowModal = (member = null) => {
        if (member) {
            ...
            this.setState({
                editingMember: null,
                formData: {
                    year: this.state.activeTab === 'staged' ? this.state.stagingYear : '2024-2025',
                    role: '',
                    role_id: '',
                    person_name: '',
                    person_email: '',
    ...
    handleInputChange = (e) => {
        ...
        if (name === 'role_id') {
            const selectedRole = this.state.availableRoles.find(role => role.id.toString() === value);
            this.setState({
                formData: {
                    ...this.state.formData,
                    role_id: value,
                    role: selectedRole ? selectedRole.role_name : '',
                    person_email: selectedRole && selectedRole.email_alias ? selectedRole.email_alias : this.state.formData.person_email
                }
            });
            return;
        }
    ...
    handleSubmit = async (e) => {
        ...
        try {
            if (this.state.editingMember) {
                await axios.put(`/api/committee/members/${this.state.editingMember.id}`, this.state.formData);
    ...
    handleStartStaging = async () => {
        const year = window.prompt('Enter the year for the new committee (e.g., 2025-2026):');
        if (!year) return;

        try {
            const response = await axios.post('/api/committee/start-staging', { year });
            ...
            const message = error.response?.data?.error || 'Error starting committee staging';
            this.showAlert(message, 'danger');
        }
    };

    handleClearStaged = async () => {
        if (!window.confirm('Are you sure you want to clear the staged committee? This will delete all staged committee members.')) {
            return;
        }
        ...
        try {
            ...
            const message = error.response?.data?.error || 'Error clearing staged committee';
            this.showAlert(message, 'danger');
        }
    };
    ...
    handleTransitionYear = async () => {
        ...
        if (!hasStagedCommittee) {
            this.showAlert('No staged committee found. Please prepare the next committee before transitioning.', 'warning');
            return;
        }

        if (!window.confirm('Are you sure you want to transition to the new committee year? This will:\n- Archive the current committee\n- Promote the staged committee to current\n\nThis action cannot be undone.')) {
            return;
        }

        try {
            await axios.post('/api/committee/transition-year');
            this.showAlert('Committee year transitioned successfully! Staged committee is now current.', 'success');
            this.setState({ activeTab: 'current', stagingYear: '' });
            this.fetchAllData();
        } catch (error) {
            console.error('Error transitioning committee year:', error);
            const message = error.response?.data?.error || 'Error transitioning committee year';
            this.showAlert(message, 'danger');
        }
    };
    ...
    render() {
        ...
        const activeCommittee = activeTab === 'current' ? currentCommittee : stagedCommittee;
                ...
                <Container>
                        ...
                        <Col md={12}>
                            <Card>
                                <Card.Header>
                                    ...
                                    <Nav variant="tabs" activeKey={activeTab} onSelect={(k) => this.setState({ activeTab: k })}>
                                        <Nav.Item>
                                            <Nav.Link eventKey="current">
                                                Current Committee
                                                <Badge bg="primary" className="ms-2">{currentCommittee.length}</Badge>
                                            </Nav.Link>
                                        </Nav.Item>
                                        <Nav.Item>
                                            <Nav.Link eventKey="staged" disabled={!hasStaged}>
                                                Staged Committee {stagingYear && `(${stagingYear})`}
                                                <Badge bg={hasStaged ? "warning" : "secondary"} className="ms-2">
                                ...
                                <Card.Body>
                                        ...
                                        <tbody>
                                            {activeCommittee.map((member) => (
                                                <tr key={member.id}>
                                                    ...
                                                    <td>{member.person_email || member.email_alias || 'N/A'}</td>
                                                    <td>{member.year}</td>
                                    ...
                                    {activeTab === 'staged' && hasStaged && (
                                        <div className="alert alert-info mt-3">
                                            <strong>Staging Mode:</strong> You are preparing the committee for {stagingYear}. 
                                            These members will become the current committee when you click "Transition to New Year".
                                        </div>
                                    )}
                                </Card.Body>
                            </Card>
                    ...
                    <Modal show={showModal} onHide={this.handleCloseModal} size="lg">
                        <Modal.Header closeButton>
                            <Modal.Title>
                                {this.state.editingMember ? 'Edit Committee Member' : 'Add Committee Member'}
                            </Modal.Title>
                        </Modal.Header>
                        <Form onSubmit={this.handleSubmit}>
                                        ...
                                        <Form.Group className="mb-3">
                                            ...
                                            {formData.role_id && (
                                                <Form.Text className="text-muted">
                                                    {(() => {
                                                        const selectedRole = this.state.availableRoles.find(r => r.id.toString() === formData.role_id);
                            ...
                            <Modal.Footer>
                                ...
                                <Button variant="primary" type="submit" disabled={loading}>
                                    {loading ? 'Saving...' : (this.state.editingMember ? 'Update' : 'Add')}
                                </Button>
                            </Modal.Footer>
                        </Form>
                    </Modal>
