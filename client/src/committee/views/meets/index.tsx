      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-zinc-500 font-medium">
            {isArchive ? 'Historical record of all club trips.' : 'Manage upcoming meets and signups.'}
          </p>
        </div>
        
        <div className="flex gap-2 w-full sm:w-auto">
          <Button asChild variant="outline" className="flex-1 sm:flex-none gap-2">
            <NavLink to={isArchive ? '/committee/meets' : '/committee/meets/archive'}>
              {isArchive ? <Clock className="h-4 w-4" /> : <Archive className="h-4 w-4" />}
              {isArchive ? 'Upcoming' : 'View Archive'}
            </NavLink>
          </Button>
          <Button asChild className="flex-1 sm:flex-none gap-2 bg-emerald-600 hover:bg-emerald-700">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-zinc-100">
                {loading ? (
                  <tr><td colSpan={5} className="px-6 py-12 text-center text-zinc-400">Loading meets...</td></tr>
                ) : sortedMeets.length > 0 ? (
                  sortedMeets.map((meet) => (
                    <tr key={meet.id} className="hover:bg-zinc-50/50 transition-colors group">
                      <td className="px-6 py-4">
                        <div className="font-bold text-zinc-900">{meet.title}</div>
                        <div className="text-[10px] text-zinc-400 font-mono uppercase">ID: {meet.id}</div>
                      </td>
                      <td className="px-6 py-4">
                        <Badge variant="outline" className="bg-zinc-100 text-zinc-600 border-none capitalize">
                          {meet.type}
                        </Badge>
                      </td>
                      <td className="px-6 py-4 text-center">
                        {meet.disabled ? (
                          <Badge className="bg-rose-50 text-rose-600 border-rose-100 hover:bg-rose-50 shadow-none">Disabled</Badge>
                        ) : (
                          <Badge className="bg-emerald-50 text-emerald-600 border-emerald-100 hover:bg-emerald-50 shadow-none">Open</Badge>
                        )}
                      </td>
                      ...
                      <td className="px-6 py-4">
                        <div className="flex justify-end gap-1">
                          <Button variant="ghost" size="sm" className="h-8 w-8 p-0" asChild title="View Details">
                            <NavLink to={`/committee/meets/view/${meet.id}`}>
                              <ExternalLink className="h-4 w-4 text-zinc-500" />
                            </NavLink>
                          </Button>
                          <Button variant="ghost" size="sm" className="h-8 w-8 p-0" asChild title="Edit Meet">
                            <NavLink to={`/committee/meets/edit/${meet.id}`}>
                              <Edit3 className="h-4 w-4 text-zinc-500" />
                            </NavLink>
                          </Button>
                          <Button variant="ghost" size="sm" className="h-8 w-8 p-0" asChild title="Clone Meet">
