...
const Treasure = () => {
  ...
  const getStatusBadge = (status: GearItem['status']) => {
    switch (status) {
      case 'Available':
        return <Badge className="bg-emerald-50 text-emerald-700 border-emerald-200 shadow-none"><CheckCircle2 className="mr-1 h-3 w-3" /> Available</Badge>;
      case 'On Loan':
        return <Badge className="bg-blue-50 text-blue-700 border-blue-200 shadow-none"><History className="mr-1 h-3 w-3" /> On Loan</Badge>;
      case 'Maintenance':
        return <Badge className="bg-amber-50 text-amber-700 border-amber-200 shadow-none"><Wrench className="mr-1 h-3 w-3" /> Maintenance</Badge>;
      default:
        ...
        <CardHeader className="border-b border-zinc-100 bg-zinc-50/50">
          <div className="flex flex-col md:flex-row gap-4 justify-between">
            ...
            <div className="flex gap-2">
              <Badge variant="outline" className="cursor-pointer hover:bg-zinc-100">All</Badge>
              <Badge variant="outline" className="cursor-pointer hover:bg-zinc-100">Technical</Badge>
              <Badge variant="outline" className="cursor-pointer hover:bg-zinc-100">Camping</Badge>
            </div>
          </div>
        </CardHeader>
                ...
                {loading ? (
                  ...
                  filteredItems.map((item) => (
                    <tr key={item.id} className="hover:bg-zinc-50/50 transition-colors group">
                      ...
                      <td className="px-6 py-4">
                        <span className="text-zinc-500 font-medium">{item.category}</span>
                      </td>
                      ...
                      <td className="px-6 py-4">
                        <span className={`text-xs font-bold ${
                          ...
                          item.condition === 'Fair' ? 'text-amber-600' : 'text-rose-600'
                        }`}>
                          {item.condition}
                        </span>
                      </td>
                  ...
                  <tr>
                    <td colSpan={5} className="px-6 py-20 text-center flex flex-col items-center justify-center gap-2">
                      <Package className="h-10 w-10 text-zinc-200" />
                      <p className="text-zinc-400 italic font-medium">No items found in the treasure chest.</p>
                    </td>
                  </tr>
                )}
              </tbody>
