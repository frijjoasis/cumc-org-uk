...
const Webmaster = ({ user, member }: WebmasterProps) => {
  ...
  const handleResetMemberships = async () => {
    if (
      !window.confirm(
        'CRITICAL ACTION: This will revoke membership from EVERYONE. Are you absolutely sure?'
      )
    )
      return;
    ...
    try {
      ...
      if (res.data.err) setStatus({ err: res.data.err, success: null });
        ...
        setStatus({
          err: null,
          success: 'All memberships have been purged successfully.',
        });
      refreshData();
    } catch (err) {
    ...
    return (
      <div className="flex items-center justify-center min-h-[60vh]">
        <Alert variant="destructive" className="max-w-md border-2">
          ...
          <AlertTitle className="font-black uppercase italic tracking-tighter">
            Access Denied
          </AlertTitle>
          <AlertDescription className="font-medium">
            Your account does not have <code>root</code> level committee
            permissions. This area is restricted to the Webmaster.
          </AlertDescription>
        </Alert>
      </div>
    );
        ...
        <Card className="md:col-span-1 border-zinc-900 bg-zinc-900 text-white overflow-hidden">
          ...
          <CardContent className="space-y-4">
            <p className="text-xs text-zinc-400 leading-relaxed italic">
              You are authenticated with global write permissions. All actions
              performed here are logged and final.
            </p>
            <div className="pt-4 border-t border-zinc-800">
              <Label className="text-[10px] uppercase font-black text-rose-500 tracking-widest block mb-3">
                Emergency Functions
              </Label>
              <Button
                variant="destructive"
          ...
          <CardHeader>
            <div className="flex justify-between items-center">
              <div>
                ...
                <CardDescription>
                  Update global committee roles and payment overrides.
                </CardDescription>
              </div>
            </div>
          </CardHeader>
                    ...
                    filteredUsers.map(u => (
                      ...
                      >
                        ...
                        <td className="px-6 py-4 text-center">
                          <Badge
                            variant="outline"
                            className={`font-mono text-[10px] ${u.member?.committee === 'root' ? 'border-rose-200 text-rose-600 bg-rose-50' : ''}`}
                          >
                        ...
                        <td className="px-6 py-4 flex justify-end">
                          <MembershipChanger
                            callback={() =>
                              refreshData(null, 'Membership updated.')
                            }
