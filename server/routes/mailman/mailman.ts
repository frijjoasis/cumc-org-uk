  ...
  fs.readFile(
    ...
    (err, data) => {
      if (!err) {
        ...
        const validLists: Array<'cumc-official' | 'cumc-freshers' | 'cumc-oldgits'> = [
          'cumc-official',
          'cumc-oldgits',
          'cumc-freshers',
        ];
        ...
        (['add', 'remove'] as Array<'add' | 'remove'>).forEach((i) => {
          validLists.forEach((l) => {
            lists[i][l] = lists[i][l].filter((e) => e !== (req.user as any).email);
            // Reset preferences

            if (req.body[i].includes(l)) lists[i][l].push((req.user as any).email);
        ...
        fs.writeFile(
          ...
          (err) => {
            if (!err) {
              logger.info('Successfully updated mail.json for ' + (req.user as any).email);
              res.json(true);
            } else {
